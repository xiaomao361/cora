...
- `v0.1.0-rc4` 是加入共享 trace Problem 关联、MCP breadcrumbs 读取边界、未处理 acknowledged
  attention 保留和签名 URL 凭据脱敏后的 Linux amd64 Server/Agent 候选构建版本；它不改变
  `gbjk-zhifu` Cora Pack 的 `cora-gbjk-v0.1.0` 经验版本。
