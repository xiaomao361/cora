...
- 同进程 `/mcp` 使用官方 Go SDK 的无状态 Streamable HTTP，提供 `cora_list_attention`、
  `cora_get_problem`、`cora_record_outcome`。所有工具显式要求产品线，详情/回写同时要求 service。
...
MCP/case、不静默失效、release identity 和本地备份恢复工具已经完成。当前 production
candidate 以 `git log -1` 为准；下一步是构建带版本的 Linux artifacts，然后进入真实
Supervisor canary，而不是继续加 UI、告警渠道或高可用改造。
...
- Cora Core、Agent、MCP/case、状态机、不静默失效信号、release identity、SQLite/positions
  备份恢复和 Supervisor production canary 文档已进入当前本地 checkpoint；以 `git log -1`
  和 `git status` 为准。
