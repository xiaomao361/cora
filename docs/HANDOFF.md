...
截至 2026-07-14，公共服务器上的 Cora Server 和一台监听 `gb-order/service02` 的 Agent 已部署。
远程只读 canary 已验证 health/readiness、四工具 MCP、attention 拉取和 Problem 详情；随后通过
...
- `internal/cora/mcp.go`：官方 Go SDK 的 Streamable HTTP MCP 三工具入口。
...
- `schemas/`：decision、evaluation row、历史 feedback 和当前 case JSON Schema。
...
- 除 `/healthz` 外的所有 Server 路径都受 bearer token 保护，`/mcp` 复用同一鉴权；
  health 仍只应位于私网监听。
- Server 默认监听收紧为 `127.0.0.1:8080`；生产以
  `cora-server -config.file=./cora.yml` 启动，严格 YAML 显式配置私网 IP、SQLite 和 token
  file，相对路径以 Supervisor 的 `/home/cora` 工作目录解析。原 CLI flags 只保留给本地调试和兼容。
...
- Problem 支持 `new / acknowledged / resolved / recurring`；已解决项不再出现在当前关注列表，
  真正晚于处理时间的新事件会转为 recurring，历史回放不会误重开。
- 同进程 `/mcp` 使用官方 Go SDK 的无状态 Streamable HTTP，提供 `cora_list_attention`、
  `cora_get_problem`、`cora_record_outcome`、`cora_export_cases`。所有工具显式要求产品线，
...
- Problem 详情通过代表样本的共享 trace ID 返回 `related_problems`，供 Agent 识别同一故障链的
  ...
  限制为首 2 条加末 6 条，每条 message 最多 768 bytes；SQLite 和 case export 保留完整原始快照。
...
中英文敏感模式及 fingerprint 不受新增上下文字段影响。面向 Agent 的 MCP 详情进一步限制
breadcrumbs 数量和单条消息大小，但不改变存储或稳定导出内容。
...
Agent/Server 可构建为静态 ELF。Server 主机只需交付 `cora-server + cora.yml`，Supervisor
固定 `/home/cora` 工作目录；Agent 主机对应 `cora-agent + agent.yml`。私网监听、目录权限、
SQLite/positions 备份与二进制回滚已形成交付文档；尚未连接真实服务器执行灰度。
...
2. **真实 canary**：部署 1 Server + 1 Agent + 1–2 个服务，连续运行至少 72 小时，完成 MCP
   拉取/回写，并以发现和闭环至少一个真实问题作为价值验收。
...
生产接入按门槛扩展：先在已接入的 `gb-order/service02` 完成 outcome 与稳定 case export，部署并
验证共享 trace 关联和 MCP breadcrumbs 边界；随后只增加 `gb-order/service01`，验证同服务跨节点
聚合。节点事实稳定后，再沿理赔案件推送链增加直接调用方和接收方，共控制在 3–5 个服务。
...
已报告部署版本新增了稳定 case 导出；下一步是先核验远端现状，再构建并部署带版本的更新
artifact，随后由本地 Hermes 验证四工具 MCP 与 72 小时 canary，而不是继续加 UI、告警渠道
或高可用改造。
...
- Cora Core、Agent、MCP/case、状态机、不静默失效信号、release identity、SQLite/positions
  备份恢复、稳定 case 导出和 Supervisor production canary 文档已进入当前 checkpoint；
  以 `git log -1`、`git status` 和 `git ls-remote origin refs/heads/main` 为准。
- `v0.1.0-rc3` 是加入共享 trace Problem 关联和 MCP breadcrumbs 读取边界后的 Linux amd64
  Server 候选构建版本；它不改变 `gbjk-zhifu` Cora Pack 的 `cora-gbjk-v0.1.0` 经验版本。
...
当前代码已达到生产 canary 候选：先检查工作树、提交 release commit，执行全量验证和
`deploy/scripts/build-release.sh <version>`，再按 `deploy/README.md` 部署 1 Server + 1 Agent。
真实主机上先完成备份恢复演练，再跑 `cora-canary` 和 72 小时观测。保持 Supervisor 单进程
边界，不先做 UI/Webhook/高可用，也不让在线结果自动改写生产规则。
```
