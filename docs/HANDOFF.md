...
截至 2026-07-14，用户已报告公共服务器上的 Cora Server 和一台监听 `gb-order/service02`
的 Agent 完成部署；本仓库尚未连接远端核验 build identity、health/readiness、positions 推进、
Problem 落库或 72 小时稳定性。本节记录部署声明，不把它等同于 canary 验收。本轮新增的
`cora_export_cases` 仍是本地未发布改动，部署中的 Server 不应被假定已包含该工具。
...
历史事件不误重开。LLM 灰区判断、case top-k 进入 Core、EWMA 突变和影响面扩大事件仍缺失。
...
- `v0.1.0-rc2` 是加入 `cora_export_cases` 后的 Linux amd64 Server 候选构建版本；它不改变
